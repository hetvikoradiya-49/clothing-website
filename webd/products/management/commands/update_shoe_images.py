"""
Management command to update existing shoe products with images from image/shoes folder.
"""
import os
import shutil
...
class Command(BaseCommand):
    help = 'Update existing shoe products with images from image/shoes folder'
    ...
    def handle(self, *args, **options):
        ...
        products_to_update = [
            {
                'product_slug': 'oxford-leather-shoes',
                'filename': 'download (2).jpg',
            },
            {
                'product_slug': 'casual-sneakers',
        ...
        for item in products_to_update:
            ...
            # Destination path
            dest_filename = f"shoe_{filename}"  # Unique name to avoid conflict
        ...
        self.stdout.write(self.style.SUCCESS('\nShoe products updated with images successfully!'))
